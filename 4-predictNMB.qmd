...
![](assets/4-predictNMB/joss.png){.absolute height="300" bottom="0"}
...
#| echo: true
#| fig-align: center
autoplot(screen_auc, dodge_width = 0.02, plot_range = F)
```
...
if(!"screen-sim-2.rds" %in% list.files("assets/4-predictNMB/data")) {
  ...
  saveRDS(screen_auc, file.path("assets/4-predictNMB/data", "screen-sim-2.rds"))
} else {
...
# AUC = 0.8
autoplot(screen_event_rate, dodge_width = 0.01, plot_range = F)
```
...
# Event rate = 0.03
ce_plot(
  screen_event_rate$simulations[[2]], ref_col = "all", add_prop_ce = TRUE
)
```
## Summary

- New software that can be used to evaluate a hypothetical prediction model
- May be used to guide research efforts towards models which are most likely to be clinically useful
